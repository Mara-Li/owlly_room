...
async def edit_ticket(ctx, idM, bot):
    ...
    if reaction.emoji == "2️⃣":
        ...
        await q.clear_reactions()
        await q.edit(
            content="De la même manière dont vous l'avez enregistré, vous pouvez indiquer un nom, ou un ID de catégorie."
        )
        rep = await bot.wait_for("message", timeout=300, check=checkRep)
        if rep.content.lower() == "stop":
            await q.delete()
...
async def edit_category(ctx, idM, bot):
    ...
    if reaction.emoji == "1️⃣":
        ...
        if config == 1:
            await q.edit(
                content="Actuellement, vous permettez le nommage libre du channel. Voulez-vous changer ce paramètre ?"
            )
            await q.add_reaction("✅")
            await q.add_reaction("❌")
            reaction, user = await bot.wait_for("reaction_add", timeout=300, check=checkValid)
        ...
        if reaction.emoji == "1️⃣":
            await q.clear_reactions()
            await q.edit(
                content="Merci de donner l'ID ou le nom de la catégorie que vous voulez ajouter."
            )
            rep = await bot.wait_for("message", timeout=300, check=checkRep)
            if rep.content.lower() == "stop":
                await q.delete()
            ...
            await q.clear_reactions()
            await q.edit(
                content="Merci de joindre l'ID ou le nom de la catégorie que vous voulez supprimer."
            )
            rep = await bot.wait_for("message", timeout=300, check=checkRep)
            if rep.content.lower() == "stop":
                await q.delete()
            ...
            chan = []
            await q.edit(
                content="Début de l'enregistrement des channels. Vous pouvez joindre un ID ou un nom de catégorie."
            )
            while True:
